...
def login():
    ...
    try:
        ...
        if redirect_response and not (300 <= redirect_response.status_code < 400):
            logger.bind(method=request.method, path=request.path, status_code=str(redirect_response.status_code), trace_id=trace_id).error("Failed to authorize redirect")
            return redirect_response
        logger.bind(method=request.method, path=request.path, status_code=str(redirect_response.status_code), trace_id=trace_id).info("Authorize redirect successfully")
        return redirect_response
...
def callback():
    ...
    try:
      ...
      logger.bind(method=request.method, path=request.path, status_code=str(200), trace_id=trace_id).info("Authorize access token successfully")
...
def logout():
    ...
    try:    # Keycloakからログアウトし、productpageにリダイレクトする
      ...
      if response and response.status_code != 302:
          logger.bind(method=request.method, path=request.path, status_code=str(response.status_code), trace_id=trace_id).error("Failed to revoke token")
          return app.make_response(redirect(url_for('front', _external=True)))
      logger.bind(method=request.method, path=request.path, status_code=str(302), trace_id=trace_id).info("Revoke token successfully")
...
def getProductDetails(product_id, headers):
    ...
    try:
        ...
        logger.bind(method="GET", path=url, trace_id=trace_id).error(f"Failed to get details: {repr(e)}")
        res = None
    if res and res.status_code == 200:
        request_result_counter.labels(destination_app='details', response_code=res.status_code).inc()
        logger.bind(method="GET", path=url, status_code=str(res.status_code), trace_id=trace_id).info("Get details successfully")
        return res.status_code, res.json()
    elif res is not None and res.status_code == 401:
        request_result_counter.labels(destination_app='details', response_code=res.status_code).inc()
        logger.bind(method="GET", path=url, status_code=str(res.status_code), trace_id=trace_id).info("Access token is invalid")
        return res.status_code, {'error': 'Please sign in to view product details'}
    elif res is not None and res.status_code == 403:
        request_result_counter.labels(destination_app='details', response_code=res.status_code).inc()
        logger.bind(method="GET", path=url, status_code=str(res.status_code), trace_id=trace_id).info("Access is denied")
        return res.status_code, {'error': 'Please sign in to view product details.'}
    elif res is not None and (res.status_code == 503  or res.status_code == 504):
        try:
          request_result_counter.labels(destination_app='details', response_code=res.status_code).inc()
          logger.bind(method="GET", path=url, status_code=str(res.status_code), trace_id=trace_id).info("Failed to get details")
          return res.status_code, res.json()
        except BaseException as e:
          logger.bind(method="GET", path=url, status_code=str(res.status_code), trace_id=trace_id).error(f"Failed to get details: {repr(e)}")
        ...
        logger.bind(method="GET", path=url, status_code=str(status), trace_id=trace_id).info("Failed to get details")
...
def getProductReviews(product_id, headers):
    ...
    try:
        ...
        logger.bind(method="GET", path=url, trace_id=trace_id).error(f"Failed to get reviews: {repr(e)}")
        res = None
    if res and res.status_code == 200:
        request_result_counter.labels(destination_app='reviews', response_code=res.status_code).inc()
        logger.bind(method="GET", path=url, status_code=str(res.status_code), trace_id=trace_id).info("Get reviews successfully")
        return res.status_code, res.json()
    elif res is not None and res.status_code == 401:
        request_result_counter.labels(destination_app='reviews', response_code=res.status_code).inc()
        logger.bind(method="GET", path=url, status_code=str(res.status_code), trace_id=trace_id).info("Access token is invalid")
        ...
        try:
          request_result_counter.labels(destination_app='reviews', response_code=res.status_code).inc()
          logger.bind(method="GET", path=url, status_code=str(res.status_code), trace_id=trace_id).info("Failed to get reviews")
          return res.status_code, res.json()
        except BaseException as e:
          logger.bind(method="GET", path=url, trace_id=trace_id).error(f"Failed to get reviews: {repr(e)}")
        ...
        logger.bind(method="GET", path=url, status_code=str(status), trace_id=trace_id).info("Failed to get reviews")
...
def getProductRatings(product_id, headers):
    ...
    if res and res.status_code == 200:
        ...
        logger.bind(method="GET", path=url, status_code=str(res.status_code), trace_id=trace_id).info("Get ratings successfully")
        return res.status_code, res.json()
    elif res is not None and res.status_code == 401:
        request_result_counter.labels(destination_app='ratings', response_code=res.status_code).inc()
        logger.bind(method="GET", path=url, status_code=str(res.status_code), trace_id=trace_id).info("Access token is invalid")
        ...
        try:
          request_result_counter.labels(destination_app='ratings', response_code=res.status_code).inc()
          logger.bind(method="GET", path=url, status_code=str(res.status_code), trace_id=trace_id).info("Failed to get ratings")
          return res.status_code, res.json()
        except BaseException as e:
          logger.bind(method="GET", path=url, status_code=str(res.status_code), trace_id=trace_id).error(f"Failed to get ratings: {repr(e)}")
        ...
        logger.bind(method="GET", path=url, status_code=str(status), trace_id=trace_id).info("Failed to get ratings")
