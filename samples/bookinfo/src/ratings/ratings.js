    ...
    if (process.env.DB_TYPE === 'mysql') {
      ...
      connection.connect(function(err) {
          ...
          connection.query('SELECT Rating FROM ratings', function (err, results, fields) {
              if (err) {
                  logger.error({method: 'GET', path: req.url, status: 500, trace_id: traceId}, "Failed to perform select: " + err)
                  res.writeHead(500, {'Content-type': 'application/json'})
                  ...
                  logger.info({method: 'GET', path: req.url, status: 200, trace_id: traceId}, "Get ratings successfully")
                  res.writeHead(200, {'Content-type': 'application/json'})
      ...
      MongoClient.connect(url, function (err, client) {
        if (err) {
          ...
          db.collection('ratings').find({}).toArray(function (err, data) {
            if (err) {
              logger.error({method: 'GET', path: req.url, status: 500, trace_id: traceId}, "Failed to load ratings from database: " + err)
              res.writeHead(500, {'Content-type': 'application/json'})
